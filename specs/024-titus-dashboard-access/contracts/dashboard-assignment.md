...
- Generate one UUID in the application and insert it explicitly as the
  projection ID; the parameterized Drizzle SQL expression path must not rely on
  Drizzle's TypeScript-only default or a database default that does not exist.
