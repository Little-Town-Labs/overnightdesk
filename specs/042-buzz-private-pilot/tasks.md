...
- [ ] T062 Freeze the exact OCI VNIC/host-interface assignment and removal procedure, WebSocket relay URL, literal supported NIP-98 method/full-HTTPS-URL pairs, DNS-01 certificate/renewal, private resolution, exact `/32`, unchanged tailnet-policy digest, and named Buzz-intake-to-Hermes route contract, plus resource ceilings, recovery targets, approval owners, and rollback assertions in docs/runbooks/buzz-private-pilot.md (FR-004 through FR-006, FR-010, FR-011, FR-017, FR-019 through FR-021).
...
- [ ] T065 [P] Write failing private-listener, public-wildcard non-selection, DNS/certificate, no-`auth_request`, raw header/URI preservation, distinct no-`:443` WebSocket/HTTPS canonical forms, and Nginx reload contracts in infra/buzz/tests/test_ingress_contract.py (FR-004, FR-008 through FR-013, FR-022).
...
- [ ] T067 [P] Write failing three-network membership and forbidden-path tests for `buzz-ingress`, `buzz-data`, and `buzz-agents` in infra/buzz/tests/test_network_contract.py (FR-015, FR-020).
- [ ] T068 [P] Write failing secondary-address assignment/removal, exact `/32`, unchanged-policy digest, existing-address/route/Serve invariance, and listener-first rollback ordering tests in infra/buzz/tests/test_route_contract.py (FR-004 through FR-007, FR-021 through FR-023).
...
- [ ] T071 [P] Write failing per-agent identity, read/write membership, signed-owner/channel trigger, same-channel reply, bot-trigger denial, exact fail-closed Hermes mapping, cross-runtime-credential denial, existing-tool/approval-policy preservation, approval-bypass denial, network/concurrency/output/timeout/deduplication-only state, and independent-revocation tests in infra/buzz/tests/test_agent_authority_contract.py (FR-019, FR-020).
...
- [ ] T074 [US2] Implement `buzz-ingress`, `buzz-data`, and `buzz-agents` membership and internal name-resolution boundaries in infra/buzz/compose.yml and infra/buzz/compose.aegis.yml until T067 passes (FR-015, FR-020).
...
- [ ] T079 [P] [US3] Implement one route-configured Buzz intake worker with distinct Walter, Titus, and Mitchel/Trevor profiles, exact signed-owner/channel checks, same-channel bounded replies, fail-closed authenticated Hermes Runs API mapping, deduplication-only state, canonical-Nginx Buzz transport, and non-bypassable existing approval policy in infra/buzz/agents/ and infra/buzz/compose.aegis.yml until T071 passes (FR-019, FR-020).
- [ ] T080 [US2] Implement disabled install, approval-bound secondary-address assignment/removal, validate/reload, exact-route activation without policy mutation, listener-first rollback, status, approval guards, and invariant comparisons in infra/buzz/deploy-aegis.sh and infra/buzz/rollback.sh until T068 passes (FR-004 through FR-007, FR-021 through FR-023).
...
- [ ] T085 [US2] **[OWNER APPROVAL]** Disable the private listener, validate/reload, prove Buzz unreachable, withdraw only the exact `/32`, remove only the Buzz host-interface and OCI VNIC secondary-address assignment, and prove addresses, routes, policy digest, Serve, public vhosts, services, and health match baseline in specs/042-buzz-private-pilot/evidence/route-experiment.md (FR-004, FR-007, FR-008, FR-023).
...
- [ ] T089 [US2] **[OWNER APPROVAL]** Rehearse listener-first rollback, including exact route withdrawal and secondary-address removal, and confirm Buzz unreachability plus existing-state invariance in specs/042-buzz-private-pilot/evidence/rollback-rehearsal.md (FR-004, FR-021 through FR-023, SC-007).
- [ ] T090 [US1] **[OWNER APPROVAL]** After T089 has removed the rehearsal address, reassign the selected secondary private IP to the frozen OCI VNIC and host interface, repeat exact local-address/bind and public-denial proof, activate only its `/32` route/listener without changing tailnet policy, and admit only the client-held owner identity (FR-004 through FR-006, FR-009, FR-018, FR-021).
...
- [ ] T093 [US3] **[OWNER APPROVAL]** Execute twenty valid, caller/channel/bot-trigger denial, signature, exact-route, existing-tool/approval-policy, direct-relay/store denial, duplicate, adversarial, restart, resource, and independent-revocation cases for the canary in specs/042-buzz-private-pilot/evidence/canary-qualification.md (SC-005, SC-009 through SC-012).
