...
**Status**: Blocked at Gate 0 on T057 object-store qualification; no production
mutation authorized
...
- [ ] T057 **[BLOCKED]** Requalify relay, PostgreSQL, Redis, one maintained S3-compatible object store and its initializer, and the Hermes intake-worker base by immutable ARM64 digest, provenance, SBOM, vulnerability disposition, non-root/read-only-root execution, persistence, and startup behavior in specs/042-buzz-private-pilot/evidence/current-images.md (FR-001, FR-016, FR-026). The historical MinIO images, Garage v2.3.0, RustFS `1.0.0-rc.5`, unsupported no-S3 operation, and probe-disable are rejected as correctness waivers. Progress requires a newer RustFS release or a different maintained candidate that passes the unchanged Gate 0 requirements in contracts/object-store.md; passing T057 makes it only a provisional local candidate. T070 and T081 must later prove the exact Buzz path-style, conditional-write, range, multipart, listing, metadata/tag, object-version, media/Git, community-deletion, storage-sweep, restart, and recovery contracts before final selection. Final first-party intake-worker image qualification occurs in T081 after T079 creates it.
...
- [ ] T070 [P] Write failing selected-object-store operation tests for bucket bootstrap/private access, path-style addressing, PUT/GET/HEAD/DELETE, byte ranges, multipart transfer, paginated listing, tags/metadata, conditional `If-Match`/`If-None-Match`, version listing/exact-version deletion, default Git conformance, media/Git/community-deletion/storage-sweep paths, and restart persistence, plus coherent PostgreSQL+object-store set, completeness, disposable restore, and logical-assertion tests in infra/buzz/tests/test_backup_restore_contract.py (FR-016, FR-017).
...
- [ ] T073 [US2] Implement immutable, non-root, hardened relay, PostgreSQL, Redis, selected qualified object-store, and minimal private-bucket initializer services, volumes, health checks, and limits in infra/buzz/compose.yml and infra/buzz/compose.aegis.yml until T064 and the T070 object-store operation contracts pass (FR-001, FR-003, FR-016, FR-025).
...
- [ ] T078 [P] [US2] Implement coherent encrypted PostgreSQL+qualified-object-store backup and disposable restore helpers in infra/buzz/backup-buzz.sh and infra/buzz/restore-rehearsal.sh until the remaining T070 recovery contracts pass (FR-016, FR-017).
...
- [ ] T081 [US2] Qualify the final intake-worker image by immutable ARM64 digest, provenance, SBOM, vulnerability disposition, non-root execution, and startup behavior; run all local contracts, the selected object store's exact Buzz operation/default-probe matrix, rendered-Compose checks, synthetic protocol matrix, safe-log sentinel, recovery rehearsal, and rollback simulation; record digest-bound results in specs/042-buzz-private-pilot/evidence/local-qualification.md (FR-001, FR-016, SC-001, SC-005, SC-012).
