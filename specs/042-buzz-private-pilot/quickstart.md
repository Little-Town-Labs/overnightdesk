...
5. Freeze the exact VNIC/interface assignment and removal procedure, WebSocket
   ...
   tailnet-wide policy digest, named Hermes identities, limits, recovery
   targets, approval owners, and rollback assertions.
...
owner-only automated triggers, coherent restore, safe logs, and listener-first
rollback ordering.
...
6. Disable the listener, validate/reload, prove Buzz unreachable, withdraw the
   `/32`, remove only the Buzz host/VNIC address assignment, and prove
...
denial, restart, capacity, and queued/in-flight revocation.
