...
const (
	...
	briefContract       = `The child output MUST be one raw JSON object (no Markdown fences) with exactly these keys: schemaVersion (exactly "meeting-brief/v1"), title, occurredAt, participants, summary, facts, decisions, actionItems, externalCommitments, unresolvedQuestions, proposedFollowUp, projectHint, and projectConfidence. actionItems MUST contain only title, owner (gary, austin, or unassigned), dueDate, sourceTimestamp, and confidence; externalCommitments MUST contain only title, dueDate, sourceTimestamp, and confidence. Use null for an unknown projectHint or dueDate, and use projectConfidence unknown, low, medium, or high. The exact shape is {"schemaVersion":"meeting-brief/v1","title":"...","occurredAt":"...","participants":[],"summary":"...","facts":[],"decisions":[],"actionItems":[],"externalCommitments":[],"unresolvedQuestions":[],"proposedFollowUp":"","projectHint":null,"projectConfidence":"unknown"}. Do not use legacy keys schema, discussion, actions, followUp, meetingReference, version, subject, purpose, key_points, risks_or_issues, audience, confidence, or action_items.`
	qaContract          = `The Sol response MUST be one raw JSON object (no Markdown fences) with exactly these keys: schemaVersion (exactly "meeting-qa/v1"), status (QA_PASS or QA_BLOCKED), meetingReference, attempt, sourceDigest, draftAttempts, qaReviews, and either brief or safeReasonCode. The exact envelope shape is {"schemaVersion":"meeting-qa/v1","status":"QA_BLOCKED","meetingReference":"MB-...","attempt":1,"sourceDigest":"...","draftAttempts":1,"qaReviews":1,"safeReasonCode":"schema_failed"}. QA_PASS MUST include brief using the exact child object unchanged and MUST omit safeReasonCode. QA_BLOCKED MUST include only an allowlisted safeReasonCode and MUST omit brief. Do not use legacy key schema or any other keys.`
)
