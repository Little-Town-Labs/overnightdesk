...
require_pattern '^name: titus-project-knowledge$' \
  "$tenant_root/skills/titus-project-knowledge/SKILL.md"
require_pattern '/opt/data/project-briefs' \
  "$tenant_root/skills/titus-project-knowledge/SKILL.md"
...
require_pattern 'MEMORY_TENCENTDB_LLM_MODEL' "$tenant_root/runtime/load-phase-env.sh"
...
require_pattern 'provider: openai-codex' "$tenant_root/config/config.yaml"
require_pattern 'base_url: "https://chatgpt\.com/backend-api/codex"' "$tenant_root/config/config.yaml"
require_pattern 'model: "gpt-5\.6-luna"' "$tenant_root/config/config.yaml"
require_pattern 'reasoning_effort: high' "$tenant_root/config/config.yaml"
require_pattern 'subagent_auto_approve: false' "$tenant_root/config/config.yaml"
require_pattern 'gpt-5\.6-sol' "$tenant_root/runtime/load-phase-env.sh"
require_pattern 'xiaomi/mimo-v2\.5-pro' "$tenant_root/runtime/load-phase-env.sh"
require_pattern 'HERMES_INFERENCE_MODEL' "$tenant_root/runtime/start-with-secrets.sh"
require_pattern 'TDAI_LLM_MODEL=\$MEMORY_TENCENTDB_LLM_MODEL' "$tenant_root/runtime/start-with-secrets.sh"
reject_pattern 'TDAI_LLM_MODEL=\$HERMES_DEFAULT_MODEL' "$tenant_root/runtime/start-with-secrets.sh"
...
require_pattern 'gpt-5\.6-sol' "$tenant_root/scripts/deploy-aegis.sh"
require_pattern 'gpt-5\.6-luna' "$tenant_root/scripts/deploy-aegis.sh"
require_pattern 'memory_llm_route=xiaomi/mimo-v2\.5-pro' "$tenant_root/scripts/deploy-aegis.sh"
require_pattern 'active_provider.*openai-codex' "$tenant_root/scripts/deploy-aegis.sh"
require_pattern 'auth_mode.*chatgpt' "$tenant_root/scripts/deploy-aegis.sh"
require_pattern 'st_mode.*0o600' "$tenant_root/scripts/deploy-aegis.sh"
reject_pattern 'x-ai/grok-build-0\.1' "$tenant_root/config/config.yaml"
reject_pattern 'x-ai/grok-build-0\.1' "$tenant_root/runtime/start-with-secrets.sh"
reject_pattern 'x-ai/grok-build-0\.1' "$tenant_root/scripts/deploy-aegis.sh"
reject_pattern 'x-ai/grok-build-0\.1' "$tenant_root/README.md"
...
require_pattern 'TELEGRAM_ALLOW_ALL_USERS=false' "$tenant_root/runtime/start-with-secrets.sh"
require_pattern 'group_allow_from' "$tenant_root/runtime/start-with-secrets.sh"
reject_pattern 'TELEGRAM_WEBHOOK_URL|TELEGRAM_WEBHOOK_SECRET' "$tenant_root/runtime/start-with-secrets.sh"
require_pattern 'group_allow_from: \[\]' "$tenant_root/config/config.yaml"
...
require_pattern 'Treat an ordinary-language request to send email as the command' "$tenant_root/skills/agentmail-email/SKILL.md"
require_pattern 'Approve and send this email?' "$tenant_root/skills/agentmail-email/SKILL.md"
require_pattern 'Never ask the owner to name or call an MCP tool' "$tenant_root/skills/agentmail-email/SKILL.md"
require_pattern 'Tool names, approval tokens, and fingerprints are internal controls' "$tenant_root/skills/agentmail-email/SKILL.md"
require_pattern 'Never reinterpret `ambiguous_unverified` as a false alarm' "$tenant_root/skills/agentmail-email/SKILL.md"
...
require_pattern 'TITUS_TELEGRAM_STATE' "$tenant_root/scripts/deploy-aegis.sh"
require_pattern 'telegram_state=' "$tenant_root/scripts/deploy-aegis.sh"
require_pattern 'TELEGRAM_ALLOWED_USERS' "$tenant_root/scripts/deploy-aegis.sh"
