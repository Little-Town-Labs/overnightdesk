...
prepare() {
  ...
  "${ssh_cmd[@]}" '
    ...
    sudo rm -f \
      /etc/systemd/system/obsidian-sync-titus.service \
      /opt/hermes-titus/bin/load-obsidian-sync-env.sh \
      /opt/hermes-titus/bin/initialize-obsidian-sync.sh \
      /opt/hermes-titus/bin/prepare-obsidian-sync.sh \
      /opt/hermes-titus/bin/prepare-project-knowledge.sh \
      /opt/hermes-titus/bin/run-obsidian-sync.sh \
      /opt/hermes-titus/bin/stop-obsidian-sync.sh \
      /run/obsidian-sync-titus/runtime.env
    sudo rm -f \
      /opt/hermes-titus/source/runtime/load-obsidian-sync-env.sh \
      /opt/hermes-titus/source/runtime/initialize-obsidian-sync.sh \
      /opt/hermes-titus/source/runtime/prepare-obsidian-sync.sh \
      /opt/hermes-titus/source/runtime/run-obsidian-sync.sh \
      /opt/hermes-titus/source/runtime/stop-obsidian-sync.sh \
      /opt/hermes-titus/source/runtime/obsidian-sync-titus.service
    sudo find /opt/hermes-titus/source/obsidian-sync -mindepth 1 -delete 2>/dev/null || true
    sudo rmdir /opt/hermes-titus/source/obsidian-sync/bin 2>/dev/null || true
    sudo rmdir /opt/hermes-titus/source/obsidian-sync 2>/dev/null || true
    sudo find /run/obsidian-sync-titus -mindepth 1 -delete 2>/dev/null || true
    sudo rmdir /run/obsidian-sync-titus 2>/dev/null || true
...
verify() {
  "${ssh_cmd[@]}" '
    ...
    sudo docker inspect -f "{{range .Mounts}}{{println .Name .Destination .RW}}{{end}}" hermes-titus |
      grep -Fq "titus-project-knowledge-data /opt/data/project-briefs true"
    sudo docker exec hermes-titus test -f /opt/data/skills/titus-project-knowledge/SKILL.md
    sudo docker exec hermes-titus /usr/bin/bash -euo pipefail -c '\''
      canary=/opt/data/project-briefs/.titus-write-check
      printf "%s\n" verified >"$canary"
      grep -Fxq verified "$canary"
      rm -f "$canary"
    '\''
