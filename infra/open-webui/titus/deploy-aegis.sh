...
usage() {
  printf 'usage: %s {prepare|install-disabled|reconcile-persona|verify-private|enable-route|verify-public|sentinel-logs|rollback|status}\n' "$0" >&2
  exit 2
}
...
prepare() {
  ...
  rsync -az -e "ssh -i $ssh_key" "$repo_root/infra/open-webui/common/" "$remote:/tmp/open-webui-titus-deploy/common/"
  "${ssh_cmd[@]}" '
    set -eu
    ...
    sudo install -o root -g root -m 0755 /opt/open-webui-titus/source/common/seed-persona-model.sh /opt/open-webui-titus/bin/seed-persona-model.sh
    sudo install -o root -g root -m 0755 /opt/open-webui-titus/source/common/verify-persona-model.sh /opt/open-webui-titus/bin/verify-persona-model.sh
    sudo install -o root -g root -m 0555 /opt/open-webui-titus/source/common/seed_persona_model.py /opt/open-webui-titus/bin/seed_persona_model.py
...
reconcile_persona() {
  prepare
  "${ssh_cmd[@]}" 'sudo systemctl restart open-webui-titus.service'
  verify_private
}
...
verify_private() {
  "${ssh_cmd[@]}" '
    ...
    sudo env \
      ...
      OPEN_WEBUI_DATA_VOLUME=open-webui-hermes-titus-data \
      OPEN_WEBUI_PERSONA_CONFIG=/opt/open-webui-titus/source/persona-model.json \
      OPEN_WEBUI_PERSONA_SEED_SCRIPT=/opt/open-webui-titus/bin/seed_persona_model.py \
      /opt/open-webui-titus/bin/verify-persona-model.sh
