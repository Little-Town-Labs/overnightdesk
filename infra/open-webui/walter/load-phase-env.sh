...
token_file=${PHASE_TOKEN_FILE:-/opt/overnightdesk/secrets/phase-service-token}
runtime_dir=${OPEN_WEBUI_RUNTIME_DIR:-/run/open-webui-walter}
output_file=${OPEN_WEBUI_RUNTIME_ENV:-/run/open-webui-walter/runtime.env}
phase_app=${OPEN_WEBUI_PHASE_APP:-overnightdesk}
phase_env=${OPEN_WEBUI_PHASE_ENVIRONMENT:-production}
phase_path=${OPEN_WEBUI_PHASE_PATH:-/agents/open-webui/hermes-walter}

die() {
  printf 'Walter Open WebUI phase load: %s\n' "$*" >&2
  exit 1
}
...
install -d -o root -g open-webui-walter -m 0750 "$runtime_dir"
...
jq -e 'type == "object" and keys == ["OPENAI_API_KEY", "WEBUI_SECRET_KEY"]' \
  "$work_dir/secrets.json" >/dev/null || die 'unexpected key in Walter Open WebUI Phase path'
jq -e '
  (.OPENAI_API_KEY | type == "string" and length >= 32) and
  (.WEBUI_SECRET_KEY | type == "string" and length >= 32)
' "$work_dir/secrets.json" >/dev/null || die 'required Walter Open WebUI secret is unavailable'

{
  jq -r 'to_entries[] | "\(.key)=\(.value | @sh)"' "$work_dir/secrets.json"
  cat <<'CONFIG'
WEBUI_URL=https://walter-chat.overnightdesk.com
WEBUI_NAME=OvernightDesk Walter
OPENID_PROVIDER_URL=https://www.overnightdesk.com/api/auth/.well-known/openid-configuration
OPENID_REDIRECT_URI=https://walter-chat.overnightdesk.com/oauth/oidc/callback
OAUTH_CLIENT_ID=overnightdesk-open-webui-walter-v1
OAUTH_CLIENT_SECRET=''
...
OPENAI_API_BASE_URL=http://hermes-walter:8642/v1
...
install -o root -g open-webui-walter -m 0440 "$work_dir/runtime.env" "$output_file"
printf 'Walter Open WebUI phase load: ready\n'
